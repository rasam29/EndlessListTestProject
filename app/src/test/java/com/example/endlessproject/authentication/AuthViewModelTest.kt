...
import com.example.endlessproject.tools.MainDispatcherRule
import com.example.endlessproject.tools.getOrAwaitValue
...
class AuthViewModelTest {
    ...
    fun before() {
        ...
        viewModel = AuthViewModel(sharedPref, repository)
    }

    @Test
    fun `when failed get Failed`() = runTest {
        coEvery { repository.getNumberList() }.returns(Either.Left(Failure("Test")))
        viewModel.authenticate()
        assertEquals(viewModel._handleFailure.getOrAwaitValue().message, "Test")
    }
    @Test
    fun `successfully authentication`() {
        val pair = Pair(2, 3)
        coEvery { repository.getAuthKey(pair.first * pair.second) }.returns(Either.Right("Test"))
        assertTrue(!viewModel.isAuthLoading.getOrAwaitValue())
        viewModel.getAuthKey(pair)
        assertTrue(viewModel.isAuthLoading.getOrAwaitValue())
    }
